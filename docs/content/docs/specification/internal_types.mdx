...
An Edge defines a connection from one node to another. Edges live in the
node's `to` field and are followed after the node produces its output.
...
<TypeTable
  type={{
    ref: {
      type: "string",
      description:
        "Reference to the target node in the 'Kind/name' format. The kind is required. ToolNode cannot be a target - use the 'tools' field instead.",
      required: true,
    },
    when: {
      type: "LimanCE | FunctionRef",
      description:
        "Condition that must be met for the edge to be followed. When omitted, the edge is always followed.",
      required: false,
    },
  }}
/>

The `nodes` and `llm_nodes` fields are supported as an alternative form.
Bare names in them get the kind from the field itself: `nodes` targets `Node`,
`llm_nodes` targets `LLMNode`. A spec cannot use `to` together with `nodes`
or `llm_nodes` - this is a validation error.
